...
#[serde(rename_all = "camelCase")]
struct PushToSpotifyRequest {
    name: Option<String>,
    public: Option<bool>,
}

pub(super) struct PushToSpotifyResult {
...
/// Push a local playlist to Spotify. Creates a Spotify playlist with the same tracks,
/// links both rows via canonical_playlist_id. Returns the Spotify URL.
pub(super) async fn push_playlist_to_spotify(
    state: &Arc<AppState>,
    local_playlist_id: i64,
    name_override: Option<&str>,
    public: bool,
) -> Result<PushToSpotifyResult, (StatusCode, serde_json::Value)> {
    // 1. Fetch playlist name
    let playlist_name: String =
        sqlx::query_scalar("SELECT name FROM service_playlists WHERE id = ?")
            ...
            .ok_or((
                StatusCode::NOT_FOUND,
                serde_json::json!({"error": "Playlist not found"}),
            ))?;

    let name = name_override.unwrap_or(&playlist_name);

    // 2. Check canonical_playlist_id — if already linked to Spotify, return 409
    let existing_canonical: Option<String> =
        sqlx::query_scalar("SELECT canonical_playlist_id FROM service_playlists WHERE id = ?")
            .bind(local_playlist_id)
            .fetch_optional(&state.db)
            .await
            .unwrap_or(None)
    ...
    if let Some(ref cid) = existing_canonical {
        let spotify_exists: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM service_playlists WHERE canonical_playlist_id = ? AND service = 'spotify'"
        ).bind(cid).fetch_one(&state.db).await.unwrap_or(0);
        if spotify_exists > 0 {
            return Err((
                StatusCode::CONFLICT,
                serde_json::json!({"error": "Playlist already pushed to Spotify"}),
            ));
        }
    }

    // 3. Resolve Spotify track URIs
    let track_uris: Vec<String> = sqlx::query_scalar(
        "SELECT 'spotify:track:' || st.service_id FROM service_playlist_tracks spt
         JOIN service_tracks st ON st.id = spt.track_id AND st.service = 'spotify'
    ...
    if track_uris.is_empty() {
        return Err((
            StatusCode::BAD_REQUEST,
            serde_json::json!({"error": "No Spotify tracks in playlist"}),
        ));
    }
    ...
    let client = match SpotifyClient::from_stored_tokens(state.db.clone(), &state.config).await {
        ...
        Err(e) => {
            return Err((
                ...
                serde_json::json!({"error": format!("Spotify not configured: {}", e)}),
            ));
        }
    };

    // 5. Get user ID & create playlist
    let user_id = client.get_current_user_id().await.map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            serde_json::json!({"error": format!("{}", e)}),
        )
    ...
    let (spotify_id, spotify_url) = client.create_playlist(&user_id, name, public,
        Some("Generated by Momo's Music Manager")).await
        .map_err(|e| {
            let err_str = format!("{}", e);
            if err_str.contains("403") {
                (StatusCode::FORBIDDEN, serde_json::json!({"error": "Spotify token needs write permissions. Re-authenticate on the Services page.", "needsReauth": true}))
            } else {
                (StatusCode::INTERNAL_SERVER_ERROR, serde_json::json!({"error": err_str}))
            }
        })?;
    ...
    client
        .add_tracks_to_playlist(&spotify_id, &track_uris)
        .await
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                serde_json::json!({"error": format!("Failed to add tracks: {}", e)}),
            )
        })?;

    // 7. Link via canonical_playlist_id
    let (canonical_id, is_new_canonical) = if let Some(cid) = existing_canonical {
        (cid, false)
    } else {
        (Uuid::new_v4().to_string(), true)
    };
    if is_new_canonical {
        sqlx::query("UPDATE service_playlists SET canonical_playlist_id = ? WHERE id = ?")
            .bind(&canonical_id)
            .bind(local_playlist_id)
            .execute(&state.db)
            .await
...
async fn push_to_spotify_handler(
    State(state): State<Arc<AppState>>,
    Path(id): Path<i64>,
    Json(request): Json<PushToSpotifyRequest>,
) -> impl IntoResponse {
    match push_playlist_to_spotify(
        &state,
        id,
    ...
    .await
    {
        Ok(result) => Json(ApiResponse {
            data: serde_json::json!({
                "playlistId": id,
                "spotifyPlaylistId": result.spotify_playlist_id,
                "spotifyUrl": result.spotify_url,
                "tracksPushed": result.tracks_pushed,
            }),
        })
        .into_response(),
...
pub(super) fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/api/playlists/{id}/push-to-spotify",
            post(push_to_spotify_handler),
        )
}
