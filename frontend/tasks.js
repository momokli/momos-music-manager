...
function renderTypeBadge(taskType) {
    const cls = taskType === 'spotify_sync' ? 'spotify_sync' : 'write_comment';
    const icon = taskType === 'spotify_sync' ? '<i class="fab fa-spotify"></i>' : '<i class="fas fa-pen"></i>';
    const label = taskType === 'spotify_sync' ? 'Spotify Sync' : 'Write Comment';
...
function renderTaskRow(task) {
    ...
    const showLogsBtn = task.logs && task.logs.length > 0
...
function showLogs(taskId) {
    ...
    apiFetch(`${API_BASE}/tasks/${taskId}`).then(response => {
