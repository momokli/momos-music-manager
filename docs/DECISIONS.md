...
## ADR-060: Backpack-Playlist ist abgeleiteter Zustand (Mirror statt Append)

**Date**: 2026-09-21
**Status**: Accepted (implemented)

**Context**: Die Backpack-Menge (Union aus aktiven Playlist-Subscriptions und
`tags.backpack = 1`) wird in **eine** Spotify-Playlist `Backpack` materialisiert;
nur diese eine URL wird an deemix submittet (Backpack-Verschmelzung, PR #34).
Die ursprüngliche Implementierung war ein Nebeneffekt des Subscription-Pollers
und benutzte `playlist_add_items` (append). Folge: Wurde ein Track aus dem
Backpack entfernt, blieb er **für immer** in der Spotify-Playlist → deemix lud
ihn weiter (Residual-Downloads), die Playlist driftete vom Backpack weg.
Zusätzlich gab es keinen manuellen Trigger und keine Verifikation — bei
Teilfehlern wurde die Signatur trotzdem als „synchron" gesetzt.

**Decision**: Die Spotify-Playlist ist **abgeleiteter** (nicht primärer) Zustand
und wird deshalb **vollständig ersetzt**, nie ergänzt:

1. **Mirror-Semantik** — `SpotifyClient::replace_playlist_items` (Chunks ≤ 100)
   statt `add_tracks_to_playlist`. Die Remote-Playlist entspricht danach exakt
   dem Backpack-Set.
2. **Re-Create-Guard** — ist die gespeicherte `playlist_id` nicht mehr
   authorbar (404/403: manuell gelöscht, Account gewechselt), wird sie neu
   angelegt und die Settings-Keys werden aktualisiert. Nie wedgen.
3. **Verifikation ist Teil der Materialisierung** — nach dem Replace wird die
   Remote-Playlist zurückgelesen (`get_playlist_track_uris`, paginated; lokale
   und Episode-Items aussortiert) und als Menge mit der intendierten verglichen.
   Nur bei Gleichheit wird `backpack.signature` fortgeschrieben.
4. **Trigger ist explizit** — Mutationen (Track-Toggle, Tag-Flag,
   Subscribe/Unsubscribe, Global-Poller-Snapshot-Änderung) setzen einen
   **Dirty-Marker** (`backpack.dirty_at`); der `BackpackSyncCoordinator`
   materialisiert nach Debounce (30 s) und zusätzlich als Safety-Net alle 10 min.
   `POST /api/backpack/push` (`force` / `dryRun` / `submitToDeemix`) erzwingt
   einen sofortigen Push mit sichtbarem Feedback.
5. **Genau ein deemix-Submit** — es bleibt bei genau einer
   `deemix_downloads`-Row (`is_backpack = 1`). `submit_to_deemix=false` baut nur
   die Playlist.

**Consequences**:

- Manuelle Handarbeit in der Spotify-Playlist geht bei jedem Push verloren —
  akzeptiert, die Playlist ist dokumentiert MMM-authored (`private`,
  Beschreibung „generated by MMM").
- Push + Verifikation kosten 2 Spotify-Calls (+1 pro 100 Tracks); Debounce und
  Signatur-Gate halten das klein, der bestehende 429-Retry greift.
- Ein Verifikations-Mismatch setzt die Signatur **nicht**; der nächste Lauf
